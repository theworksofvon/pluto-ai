...
class BaseRetriever():
    ...
    def __init__(self, parser: Optional[LlamaParse] = None, embed_model: Optional[Any] = None, llm: Optional[Any] = None) -> None:
        self.parser = parser or LlamaParse(result_type="markdown", api_key=config.LLAMA_CLOUD_API_KEY)
        self.embed_model = embed_model or HuggingFaceEmbedding(model_name="sentence-transformers/all-MiniLM-L6-v2")
        self.llm = llm or Ollama(model=config.OLLAMA_MODEL_NAME, base_url=config.OLLAMA_API_URL)
